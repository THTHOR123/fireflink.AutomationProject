...
public class CreateSalesOrderWithMandatoryFields {
	...
	public static void main(String[] args) throws Exception {
		...
		//opportunity details
		String nextStep = row.getCell(7).toString();
		String opportunity = row.getCell(8).toString();
		String salesStage = row.getCell(9).toString();
		String amount = row.getCell(10).toString();
		String businessType = row.getCell(11).toString();
		String leadName = row.getCell(12).toString();
		
		//quotes
		String quoteStage = row.getCell(13).toString();
		String quoteSubject = row.getCell(14).toString();
		String quoteValidDate = row.getCell(15).toString();
		
		String billingAddress = row.getCell(16).toString();
		String billingPOBox = row.getCell(17).toString();
		String billingCity = row.getCell(18).toString();
		String billingState =row.getCell(19).toString();
		String billingPostalCode = row.getCell(20).toString();
		String billingCountry = row.getCell(21).toString();
		String shippingAddress = row.getCell(22).toString();
		String shippingPOBox = row.getCell(23).toString();
		String shippingCity =row.getCell(24).toString();
		String shippingState = row.getCell(25).toString();
		String shippingPostalCode = row.getCell(26).toString();
		String shippingCountry = row.getCell(27).toString();
		String productName = row.getCell(28).toString();
		
		//product details
		String Quantity = row.getCell(29).toString();
		String price = row.getCell(30).toString();
		String category = row.getCell(31).toString();
		String vendor = row.getCell(32).toString();
		...
		driver.findElement(By.xpath("//span[text()='Create Opportunity']/ancestor::button")).click();
		
		driver.findElement(By.xpath("//input[@name='nextStep']")).sendKeys(nextStep);
		
		driver.findElement(By.xpath("//input[@name='opportunityName']")).sendKeys(opportunity);
		...
		WebElement leadSelectElement = driver.findElement(By.id("search-criteria"));
		Select leadDropDown = new Select(leadSelectElement);
		leadDropDown.selectByVisibleText("Lead Name");
		
		Thread.sleep(2000);
		driver.findElement(By.xpath("//input[@placeholder='Search']")).sendKeys(leadName.substring(0, 7));
		
//		act.keyDown(Keys.BACK_SPACE).keyUp(Keys.BACK_SPACE).perform();
		
		
		driver.findElement(By.xpath("//button[contains(@onclick,'"+ leadName+"')]")).click();
//		wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.xpath("//button[contains(text(),'"+leadName +"')]")))).click();
		...
		driver.findElement(By.xpath("//span[text()='Create Quote']/ancestor::button")).click();
		...
		driver.findElement(By.xpath("//input[@name='validTill']")).sendKeys(quoteValidDate);
		...
		while(true) {
			try {
					...
					driver.findElement(By.xpath("//td[text()='ProBook']/following-sibling::td/button")).click();
					break;
...
//				act.scrollByAmount(0, 200).perform();
		...
		Thread.sleep(2000);
		driver.findElement(By.id("search-input")).sendKeys(quoteSubject);
		
		driver.findElement(By.xpath("//button[contains(@onclick,'"+quoteSubject+"')]")).click();
